...
StrId table_addstr(StrTable *t, Slice str, int should_alloc_str) {
    if (t->occupied > (t->max_entries / 2)) {
        puts("This error occured because the table was large enough to start "
             "being "
    ...
    // inserted in this table. Pretty bold assumptions, but I don't need a
    // sanity check for every minute detail.
    while (t->entries[idx] != NULL) {
        Slice slice = *(t->entries[idx]);
        if (slice.len == str.len) {
            if (memcmp(slice.ptr, str.ptr, str.len) == 0) {
    ...
    if (should_alloc_str) {
        ...
        memcpy(alloced_str, str.ptr, str.len);
        str.ptr = alloced_str;
    }
